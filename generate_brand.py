...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        # Create SVGs with different colors
        dark_svg = os.path.join(tmpdir, "sole_dark.svg")
        light_svg = os.path.join(tmpdir, "sole_light.svg")
        write_svg(dark_svg, "#1a1a1a")
        write_svg(light_svg, "#ffffff")

        # icon.png: 256x256 square, dark logo (light theme)
        icon_path = os.path.join(BRAND_DIR, "icon.png")
        magick_convert(dark_svg, icon_path, "200x200", extent="256x256")
        print(f"Created {icon_path}")

        # dark_icon.png: 256x256 square, white logo (dark theme)
        dark_icon_path = os.path.join(BRAND_DIR, "dark_icon.png")
        magick_convert(light_svg, dark_icon_path, "200x200", extent="256x256")
        print(f"Created {dark_icon_path}")

        # logo.png: wider, dark logo (light theme)
        logo_path = os.path.join(BRAND_DIR, "logo.png")
        magick_convert(dark_svg, logo_path, "x80", extent="480x128")
        print(f"Created {logo_path}")

        # dark_logo.png: wider, white logo (dark theme)
        dark_logo_path = os.path.join(BRAND_DIR, "dark_logo.png")
        magick_convert(light_svg, dark_logo_path, "x80", extent="480x128")
        print(f"Created {dark_logo_path}")
